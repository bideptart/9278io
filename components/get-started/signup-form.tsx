          ...
          <div className="grid gap-3 md:grid-cols-2">
            ...
            {PHONE_NUMBER_RATES.map((r) => (
              <RegionOption
                ...
                title={r.region}
                price={`₹${r.monthlyInr.toLocaleString("en-IN")} / mo`}
          ...
          {regionRow && (
            <div className="mt-4 flex flex-wrap items-center gap-3 rounded-lg border border-border/60 bg-card/30 px-4 py-3">
              ...
              <span className="text-sm text-muted-foreground">
                × ₹{regionRow.monthlyInr.toLocaleString("en-IN")} / mo = <span className="text-foreground">₹{phoneCost.toLocaleString("en-IN")}/mo</span>
              </span>
            </div>
          )}
        ...
        >
          ...
          <dl className="mt-5 space-y-4 text-sm">
            ...
            <div className="border-t border-border/60 pt-4">
              <Row label="Voice rate" value={`₹${plan.ratePerMinInr} / min`} muted />
              <Row label="Today's charge" value={`₹${total.toLocaleString("en-IN")}`} bold />
              {phoneCost > 0 && (
                <p className="mt-2 text-xs text-muted-foreground">
                  + ₹{phoneCost.toLocaleString("en-IN")} / month thereafter for phone numbers.
                </p>
              )}
            </div>
          </dl>
