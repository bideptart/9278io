...
function CallDetailCard({ call }: { call: (typeof CALLS)[number] }) {
  ...
  return (
    <div className="flex h-[340px] flex-col overflow-hidden rounded-2xl border border-blue-100/80 bg-white/95 p-3 shadow-[0_16px_40px_-26px_oklch(0.52_0.22_265/0.3)] backdrop-blur sm:h-[375px] sm:p-4">
      {/* header */}
      <div className="flex items-center gap-2">
        <span className="flex size-8 shrink-0 items-center justify-center rounded-full bg-slate-100 text-xs font-bold text-foreground sm:size-9 sm:text-sm">
          {call.caller.charAt(0)}
        </span>
        <div className="min-w-0 flex-1 leading-tight">
          <p className="flex items-center gap-1 truncate text-xs font-bold text-foreground sm:gap-1.5 sm:text-sm">
            {call.caller} <ArrowRight className="size-3 shrink-0 text-muted-foreground sm:size-3.5" aria-hidden /> {call.title}
          </p>
          <button
            type="button"
            onClick={() => {
      ...
      {/* meta row */}
      <div className="mt-2.5 grid grid-cols-3 gap-1.5 border-t border-blue-100/70 pt-2.5 sm:mt-3 sm:gap-2 sm:pt-3">
        <div className="min-w-0 leading-tight">
          <p className="truncate text-[9px] text-muted-foreground sm:text-[10px]">Call completed</p>
          <p className="truncate text-[10px] font-semibold text-foreground sm:text-[11px]">{call.completedAt}</p>
        </div>
        <div className="min-w-0 leading-tight">
          <p className="truncate text-[9px] text-muted-foreground sm:text-[10px]">Call Duration</p>
          <p className="truncate text-[10px] font-semibold text-foreground sm:text-[11px]">{call.duration}</p>
        </div>
        <div className="min-w-0 leading-tight">
          <p className="truncate text-[9px] text-muted-foreground sm:text-[10px]">Call Status</p>
          <p className="flex items-center gap-1 text-[10px] font-semibold text-emerald-600 sm:text-[11px]">
            <CheckCircle2 className="size-3 shrink-0" aria-hidden /> {call.status}
          </p>
        </div>
      </div>

      {/* waveform player */}
      <div className="mt-2.5 flex items-center gap-2 rounded-xl border border-border bg-slate-50 px-2 py-1.5 sm:mt-3 sm:gap-2.5 sm:px-3 sm:py-2">
        <audio
          ref={audioRef}
          src={call.audioSrc}
          preload="metadata"
      ...
      {/* metric tiles */}
      <div className="mt-2.5 grid grid-cols-4 gap-1 sm:mt-3 sm:gap-1.5">
        {call.metrics.map((m) => (
          <div key={m.label} className="min-w-0 rounded-lg bg-slate-50 px-1 py-1.5 text-center leading-tight sm:px-1.5 sm:py-2">
            <p className={`truncate text-xs font-bold sm:text-sm ${m.color}`}>{m.value}</p>
            <p className="mt-0.5 truncate text-[8px] text-muted-foreground sm:text-[9px]">{m.label}</p>
          </div>
        ))}
      </div>

      {/* issues */}
      <div className="mt-2.5 flex items-center justify-between gap-2 border-t border-blue-100/70 pt-2.5 sm:mt-3 sm:pt-3">
        <p className="truncate text-[11px] font-bold text-foreground sm:text-xs">Issues across this call</p>
        <p className="shrink-0 text-[9px] text-muted-foreground sm:text-[10px]">{call.issues.length} flagged</p>
      </div>
      <div className="mt-2 flex flex-col gap-1.5 sm:gap-2">
        {call.issues.length ? (
          call.issues.map((it) => (
            <div key={it.title} className="rounded-xl border border-border bg-white px-2.5 py-1.5 sm:px-3 sm:py-2">
              ...
              <p className="mt-1 text-[10px] font-semibold text-foreground sm:text-[11px]">{it.title}</p>
              <p className="mt-0.5 text-[9px] leading-snug text-muted-foreground sm:text-[10px]">{it.desc}</p>
            </div>
          ))
        ) : (
          <div className="flex items-center gap-1.5 rounded-xl border border-emerald-100 bg-emerald-50 px-2.5 py-1.5 text-[10px] font-semibold text-emerald-600 sm:px-3 sm:py-2 sm:text-[11px]">
            <CheckCircle2 className="size-3.5 shrink-0" aria-hidden /> No issues flagged
          </div>
        )}
      </div>
    </div>
...
function HeroVisual() {
  return (
    <motion.div
      ...
      className="relative mx-auto block w-full max-w-[280px] sm:max-w-[620px]"
    >
      {/* soft glow behind */}
      <div
        ...
        <div className="grid items-stretch gap-14 lg:grid-cols-2 lg:gap-12">
          ...
          <div className="flex flex-col">
            <motion.div
              ...
              className="inline-flex w-fit items-center gap-1.5 rounded-full border border-primary/20 bg-primary/[0.07] px-3 py-1.5 text-xs font-semibold uppercase tracking-wider text-primary sm:gap-2 sm:px-4 sm:py-2 sm:text-sm"
            >
              ...
              {highlights.map((h) => {
                ...
                return (
                  <div key={h.title} className="flex flex-col items-center gap-1 bg-white px-3 py-3 text-center">
                    <div className="flex items-center gap-2">
                      <span className={`flex size-8 shrink-0 items-center justify-center rounded-full ${h.tile} ${h.color}`}>
          ...
          {/* ── Right: hero visual ── */}
          <div id="demo-audio" className="w-full scroll-mt-24 lg:pt-2">
            <HeroVisual />
          </div>
        </div>
      </div>
