...
export function LiveCallMockup({ className, framed = false }: { className?: string; framed?: boolean }) {
  const content = (
    <>
      ...
      <div className="mt-3 flex w-fit items-center gap-1.5 rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1.5 text-xs font-semibold text-emerald-700">
  ...
  // Framed mode: no card chrome, no floating rotate/drift animation, no
  // overlapping badge offset — this is meant to sit as a plain section
  // inside a single shared frame (see the mobile pricing-hero stack).
  if (framed) {
    return (
