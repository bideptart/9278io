...
export function getRegistry(): SeoEntry[] {
  const industries: SeoEntry[] = INDUSTRIES.map((i) => ({
    ...
    description:
      i.pitch ??
      `${i.name} teams use 9278.io to handle inbound calls, qualify leads, and book appointments around the clock.`,
  ...
  const internal: SeoEntry[] = [
    {
      ...
      description: "Sign in to the 9278.io super-admin dashboard.",
